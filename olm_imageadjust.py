...
class OlmImageAdjust:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "shadows": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 0.99, "step": 0.01}),
                "midtones": ("FLOAT", {"default": 1.0, "min": 0.1, "max": 3.0, "step": 0.01}),
                "highlights": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 2.0, "step": 0.01}),
                "hue": ("FLOAT", {"default": 0.0, "min": -180.0, "max": 180.0, "step": 1.0}),
                "saturation": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 2.0, "step": 0.01}),
                "value": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 2.0, "step": 0.01}),
                "vibrance": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 2.0, "step": 0.01}),
            }
        }
    ...
    def adjust_image(self, version, image: torch.Tensor, exposure, brightness, contrast, gamma,
                     shadows, midtones, highlights, hue, saturation, value, vibrance):
        debug_print("=" * 60)
        debug_print("[OlmImageAdjust] Applying image adjustments:")
        try:
            if not isinstance(image, torch.Tensor):
                raise TypeError("Input image must be a torch.Tensor")
            if image.dim() == 3:
                image = image.unsqueeze(0)
            thumbnail_cache["imageadjust_image"] = image.clone().detach()
            adjustments = {
            ...
            debug_print("=" * 60)
            return {"ui": {"message": "Image adjustments applied!"}, "result": (adjusted,)}
        except Exception as e:
            print(f"[OlmImageAdjust Error] {e}")
            return {"ui": {"message": f"Failed to apply adjustments: {e}"}, "result": (image,)}
...
async def handle_imageadjust_preview(request):
    ...
    try:
        ...
        image = load_thumbnail_image("imageadjust_image")
        ...
        return web.json_response({"status": "success", "updatedimage": f"data:image/png;base64,{img_str}"})
    except Exception as e:
...
def rgb_to_hsv(rgb: torch.Tensor):
    ...
    h[mask & (max_val == g)] = (2.0 + (b - r) / (delta + 1e-10))[mask & (max_val == g)] / 6.0
    h[mask & (max_val == b)] = (4.0 + (r - g) / (delta + 1e-10))[mask & (max_val == b)] / 6.0
    h = (h + 1.0) % 1.0
...
def load_thumbnail_image(cache_key):
    if cache_key not in thumbnail_cache:
        raise ValueError("[OlmImageAdjust] No cached image available. Please run the node first.")
    return downscale_image(thumbnail_cache[cache_key], size=(PREVIEW_RESOLUTION, PREVIEW_RESOLUTION))
...
NODE_CLASS_MAPPINGS = {
    "OlmImageAdjust": OlmImageAdjust
}


NODE_DISPLAY_NAME_MAPPINGS = {
    "OlmImageAdjust": "Olm Image Adjust"
}
